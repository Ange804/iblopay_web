            ...
            return next.handle(req).pipe(tap({
                next: (event) => {
                    if (event instanceof HttpResponse) {
                        ...
                        console.log(`%c[API] ${req.method} ${req.urlWithParams} → ${event.status} (${duration}ms)`, this.getLogStyle(event.status));
                    }
                },
                error: (error) => {
                    const duration = Date.now() - startTime;
                    console.error(`%c[API] ${req.method} ${req.urlWithParams} → ${error.status} (${duration}ms)`, 'color: #ff4444; font-weight: bold;');
                }
            }));
        }
        getLogStyle(status) {
